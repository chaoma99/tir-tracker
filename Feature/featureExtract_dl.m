...
function [feature]=featureExtract_dl(im,step)
...
% mag=bsxfun(@rdivide,mag,sqrt(sum(mag.^2))+1e-6);
% pha=bsxfun(@rdivide,pha,sqrt(sum(pha.^2))+1e-6);

mag=reshape(mag', [l1_patch_num, size(mag,1)]);
mag=conv_contrast_normalize_mc(mag,cnorm_sz,size(mag,3));
