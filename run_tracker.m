...
%
%  Learning A Temporally Invariant Features for Visual Tracking
%  
...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	otherwise
		...
		rect = [positions(:,2) - target_sz(2)/2, positions(:,1) - target_sz(1)/2];
		rect(:,3) = target_sz(2);
		rect(:,4) = target_sz(1);
        save(['./icip_result/' video 'ICIP.mat'], 'rect'); 
